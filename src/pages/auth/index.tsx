...
import { showErrorDialog, redirectTo } from '@/public/util';
...
const Auth = () => {
	...
	const handleClick = (time1) => {
		...
		setTimeout(() => {
			ap.pushWindow('alipays://platformapi/startapp?appId=20002133&jumpAction=direct-h5&schema=pinduoduo%3A%2F%2F')
		}, 200)
	...
	return (
		<div style={{ textAlign: 'center', paddingTop: 40 }}>
			...
			<Button
				color='primary'
				size='middle'
				block={false} onClick={() => handleClick(100)}>测试跳转关闭</Button>
				<div style={{ marginTop: '32px' }}></div>
			<Button
				color='primary'
				size='middle'
				block={false} onClick={() => handleClick(300)}>测试跳转关闭1</Button>
			<div style={{ marginTop: '32px' }}></div>
			<Button
				color='primary'
				size='middle'
				block={false} onClick={() => handleClick(500)}>测试跳转关闭2</Button>
		</div>
	);
};
